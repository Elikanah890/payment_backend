...
export function computeStatus(amount: Prisma.Decimal, amountPaid: Prisma.Decimal, dueDate: Date): InvoiceStatus {
  if (amountPaid.greaterThanOrEqualTo(amount)) return InvoiceStatus.PAID;
...
export class InvoiceService {
  async generate(user: AuthUser, dto: GenerateInvoiceDto) {
    ...
    if (!students.length) throw ApiError.badRequest('No matching active students');
    ...
    for (const s of students) {
      ...
      if (dto.amount != null) amount = money(dto.amount);
      else if (enr) {
        amount = money(
          enr.isHostel && enr.feePackage.hostelInstallment ? enr.feePackage.hostelInstallment : enr.feePackage.installmentAmount
        );
      } else continue;

      const invoiceNumber = await nextInvoiceNumber(prisma, schoolId);
      const now = new Date();
  ...
  async list(user: AuthUser, q: ListInvoiceQuery) {
  ...
  async getById(user: AuthUser, id: string) {
    ...
    assertSameSchool(user, inv.schoolId);
    return inv;
  }

  async adjust(user: AuthUser, id: string, dto: AdjustInvoiceDto) {
    const inv = await this.getById(user, id);
    const delta = dto.type === 'DEBIT_NOTE' ? money(dto.amount) : money(dto.amount).negated();
    let newAmount = money(inv.amount).plus(delta);
    if (newAmount.lessThanOrEqualTo(0)) throw ApiError.badRequest('Resulting invoice amount must be greater than 0');
    if (newAmount.lessThan(inv.amountPaid)) newAmount = money(inv.amountPaid);

    return prisma.$transaction(async (tx) => {
      await tx.invoiceAdjustment.create({
        data: { invoiceId: id, type: dto.type, amount: money(dto.amount), reason: dto.reason, approvedBy: user.id },
      });
      return tx.invoice.update({
        where: { id },
        data: { amount: newAmount, status: computeStatus(newAmount, money(inv.amountPaid), inv.dueDate) },
      });
    });
  }

  async waive(user: AuthUser, id: string, dto: WaiveInvoiceDto) {
    const inv = await this.getById(user, id);
    const remaining = money(inv.amount).minus(inv.amountPaid);
    if (remaining.lessThanOrEqualTo(0)) throw ApiError.badRequest('Nothing to waive');
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (paid.greaterThan(0)) {
        return tx.invoice.update({ where: { id }, data: { amount: paid, status: InvoiceStatus.PAID } });
      }
      return tx.invoice.update({ where: { id }, data: { status: InvoiceStatus.CANCELLED } });
    });
  }

  async overdue(user: AuthUser, requestedSchoolId?: string) {
  ...
  async print(user: AuthUser, id: string) {
    const inv = await this.getById(user, id);
    const school = await prisma.school.findUnique({ where: { id: inv.schoolId } });
...
<div class="footer">
  ...
  async summary(user: AuthUser, requestedSchoolId?: string) {
    const schoolId = resolveSchoolScope(user, requestedSchoolId);
    const where: Prisma.InvoiceWhereInput = schoolId ? { schoolId } : {};

    const [agg, byStatus] = await Promise.all([
      prisma.invoice.aggregate({ where, _sum: { amount: true, amountPaid: true, balance: true }, _count: true }),
      prisma.invoice.groupBy({ by: ['status'], where, _count: true }),
    ]);

    const invoiced = money(agg._sum.amount ?? 0);
    const collected = money(agg._sum.amountPaid ?? 0);
    return {
      totalInvoiced: invoiced,
      totalCollected: collected,
      totalOutstanding: money(agg._sum.balance ?? 0),
  ...
  async generatePdf(user: AuthUser, id: string): Promise<Buffer> {
    const inv = await this.getById(user, id);
    const school = await prisma.school.findUnique({ where: { id: inv.schoolId } });
    if (!school) throw ApiError.notFound('School');
